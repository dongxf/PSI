Ext.define("PSI.Inventory.InitInventoryMainForm", {
    ...
    initComponent: function () {
        ...
        Ext.apply(me, {
            ...
            layout: "border",
            tbar: [{
                    text: "建账", iconCls: "PSI-button-add", scope: me, handler: me.onInitInv
                }, "-", {
                    text: "刷新", iconCls: "PSI-button-refresh", scope: me, handler: function () {
                        me.freshInvGrid();
                    }
                }, "-",
                {
                    text: "标记建账完毕", iconCls: "PSI-button-commit", scope: me, handler: me.onFinish
                },
                {
                    text: "取消建账完毕标记", iconCls: "PSI-button-cancel", scope: me, handler: me.onCancel
                }, "-",
                {
                    text: "帮助",
                    iconCls: "PSI-help",
                    handler: function() {
                        window.open("http://my.oschina.net/u/134395/blog/375000");
                    }
                },
                "-", {
    ...
    onFinish: function () {
        ...
        PSI.MsgBox.confirm("请确认是否给仓库[" + warehouse.get("name") + "]标记建账完毕?", function () {
            var el = Ext.getBody();
            ...
            Ext.Ajax.request({
                ...
                callback: function (options, success, response) {
                    ...
                    if (success) {
                        var data = Ext.JSON.decode(response.responseText);
                        if (data.success) {
                            PSI.MsgBox.showInfo("成功标记建账完毕", function () {
                                me.freshWarehouseGrid();
                            });
                        } else {
    ...
    onCancel: function () {
        ...
        PSI.MsgBox.confirm("请确认是否取消仓库[" + warehouse.get("name") + "]的建账完毕标志?", function () {
            var el = Ext.getBody();
            el.mask(PSI.Const.SAVING);
            Ext.Ajax.request({
                ...
                callback: function (options, success, response) {
                    ...
                    if (success) {
                        var data = Ext.JSON.decode(response.responseText);
                        if (data.success) {
                            PSI.MsgBox.showInfo("成功取消建账标志", function () {
                                me.freshWarehouseGrid();
                            });
                        } else {
                            PSI.MsgBox.showInfo(data.msg);
